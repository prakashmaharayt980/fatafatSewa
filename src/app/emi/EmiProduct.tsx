...
export default function ProductEMIUI({ chooseProduct,setProductPrice }) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto my-0 p-1 sm:p-2 bg-white">
        ...
        <DrawerContent className="max-h-[85vh] min-h-[50vh] sm:min-h-[60vh] max-w-6xl mx-auto bg-white border-0 shadow-xl">
          <DrawerHeader className="text-center m-0 p-0 items-center border-b-gray-200 border-b">
            <DrawerTitle className="flex items-center justify-center gap-1 sm:gap-2 m-0 p-0 text-base sm:text-xl text-[var(--colour-fsP2)] font-semibold">
              <CreditCard className="w-4 sm:w-5 h-4 sm:h-5 text-[var(--colour-fsP1)] mb-1 sm:mb-2" />
              <span className="items-center mb-1 sm:mb-2">EMI Calculator - Select Product</span>
            </DrawerTitle>
          </DrawerHeader>
                  ...
                  {selectItems.map((product) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] sm:text-xs text-gray-600">Rs. {product.price}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {/* Main Content */}
      {product ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1 items-start sm:gap-2">
          {/* Product Image */}
          <div className="relative aspect-square p-1 sm:p-2 w-full max-w-[240px] sm:max-w-[280px] md:max-w-[360px] mx-auto">
            <Image
              src={product.image || ""}
          ...
          <div className="space-y-2 sm:space-y-3 p-1 sm:p-2 flex justify-end flex-col">
            <div>
              <label className="block text-xl sm:text-3xl capitalize font-medium text-[var(--colour-fsP2)] mb-1 sm:mb-2">
                {product.name}
              </label>
            </div>
            ...
            {uniqueColors.length > 0 && (
              <div>
                <h3 className="text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Select Color:</h3>
                <div className="flex flex-wrap gap-1 sm:gap-2">
                  {uniqueColors.map((color) => (
                    <Button
                      ...
                      className={cn(
                        "relative px-1.5 sm:px-3 py-1 sm:py-2 rounded-lg text-[10px] sm:text-sm font-medium transition-all duration-200",
                        selectedColor === color
                          ? "px-4 sm:px-6 bg-blue-900 text-white"
                    ...
                    >
                      {color}
                      {selectedColor === color && (
                        <span className="relative bg-green-500 text-white rounded-full w-3 sm:w-4 h-3 sm:h-4 flex items-center justify-center text-[8px] sm:text-xs">
                          ✓
                        </span>
                      )}
                    </Button>
            ...
            <div className="mt-4 sm:mt-6 cursor-pointer">
              <Button 
                className="px-3 sm:px-6 bg-blue-900/50 text-white w-full max-w-md justify-end flex text-sm sm:text-base" 
        ...
        <div className="mt-4 sm:mt-6">
          <Button
            className={cn(
              "w-full text-xl sm:text-2xl min-h-[150px] sm:min-h-[200px] mx-auto border-2 border-dashed border-gray-400 bg-white text-gray-700 py-2 sm:py-3 rounded-lg font-semibold",
