...
import { ProductTrending } from '../types/CategoryTypes';
...
const fetcher = async () => {
  const response = await RemoteServices.ProductTranding();
  return response.data; // Assuming RemoteServices.ProductTrending returns { data: ProductTrending[] }
};

const BasketCardTrading = ({ title }: BasketCardTradingProps) => {
  ...
  const { data: productList, error } = useSWR<ProductTrending[]>(
    inView ? 'trending-products' : null, // Use a static key since no slug is passed
    fetcher,
    {
  ...
  return (
    <div ref={ref} className="w-full bg-white overflow-visible">
      <div className={cn('flex items-center justify-between py-1', 'px-3 sm:px-6')}>
        ...
        <button
          onClick={() => router.push(`/product/category/${productList[0]?.slug}`)}
          className={cn(
