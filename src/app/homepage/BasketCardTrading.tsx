...
import { ProductTrending } from '../api/ContextStore';
...
const BasketCardTrading = ({ title, items }:BasketCardTradingProps ) => {
    const router = useRouter();
    const scrollContainerRef = useRef<HTMLDivElement>(null);
    const [activeDot, setActiveDot] = useState(0);
    const [windowWidth, setWindowWidth] = useState(0);

    console.log('items in basket trading:', items);
   
    const Datalist = items
    const ItemCatg = items
    ...
    const itemsPerPage = isMobile ? 2 : 6; // 2 items on mobile, 5 on desktop
    ...
    return (
        <div className="w-full bg-white overflow-visible"> {/* Added overflow-visible to parent */}
            ...
            <div className={cn(
                ...
                <button
                    onClick={() => router.push(`/product/category/${ItemCatg[0]?.slug}`)}
                    className={cn(
                    ...
                    {Datalist.map((product, index) => (
                        <div
                            ...
                            className={cn(
                                ...
                                // Desktop: each item takes ~15% width (6 items visible)
                                'w-[calc(50%-8px)] sm:w-[calc(16.666%-6px)]'
                            )}
                        >
